...
Rebot:
	...
	for (;is_run();delay_fps(FPS_FULL))//改成FPS_FULL可以改善Hover延迟，但是依然存在
	{
		while (mousemsg())//注意把判断放在外面避免重复判断
		...
		while (kbhit())//所以kbmsg和kbhit还有区别！这里必须用kbhit（也是windows下的比较官方的函数了）
...
//EOF 2023-11-12 21:48：基本实现输入功能和按钮的兼容，但是依然存在按下按键才能响应鼠标的问题，目前暂时无法修复